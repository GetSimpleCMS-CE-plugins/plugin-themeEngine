...
<div class="lister">
	...
	$result = $db->query('SELECT * FROM items ORDER BY item_order ASC');
...
<form class="former-te" method="POST">
	...
	while ($row = $result->fetchArray(SQLITE3_ASSOC)) {
		...
		if ($row['type'] == 'text') {
			echo '<textarea  class="te-text" name="value[]" style="height:40px">' . $row['value'] . '</textarea>';
		}
		...
		if ($row['type'] == 'textarea') {
			echo '<textarea class="te-textarea" name="value[]">' . $row['value'] . '</textarea>';
		}

		if ($row['type'] == 'ckeditor') {
			echo '<textarea id="post-content" name="value[]">' . $row['value'] . '</textarea>';
		}
		...
		if ($row['type'] == 'link') {
			...
			foreach (glob(GSDATAPAGESPATH . "*.{xml}", GLOB_BRACE) as $page) {
				...
				global $SITEURL;
				global $USR;
				echo "<option value='" . $path_parts['filename'] . "' " . ($row['value'] == $path_parts['filename'] ? 'selected' : '') . "  >" . $path_parts['filename'] . "</option>";
			}
			;
...
<footer>
	<p class="w3-small clear w3-margin-bottom w3-margin-left">Made with <span class="credit-icon">❤️</span> especially
		for "<b><?php echo $USR; ?></b>". Is this plugin useful to you?
		<span class="w3-btn w3-khaki w3-border w3-border-red w3-round-xlarge"><a href="https://getsimple-ce.ovh/donate"
				target="_blank" class="donateButton"><b>Buy Us A Coffee </b><svg xmlns="http://www.w3.org/2000/svg"
					style="vertical-align:middle" width="24" height="24" viewBox="0 0 24 24">
					...
					<g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
						...
						<rect width="24" height="0" y="7" fill="currentColor"
...
if (isset($_POST['saveToDB'])) {
	try {
		...
		// Sprawdź, czy wszystkie tablice mają tę samą długość
		$count = count($_POST['name']);
		if (
			count($_POST['id']) !== $count ||
			count($_POST['type']) !== $count ||
			count($_POST['value']) !== $count
		) {
			throw new Exception('Niezgodność danych w formularzu.');
		}
		...
		foreach ($_POST['name'] as $key => $name) {
			...
			if ($slugCheckResult) {
				throw new Exception('Slug "' . htmlspecialchars($slug, ENT_QUOTES, 'UTF-8') . '" jest już w użyciu.');
			}

			// Walidacja dla dropdown
			if ($type === 'dropdown' && !empty($value)) {
				$options = array_map('trim', explode('||', $row['value']));
				if (!in_array($value, $options)) {
					throw new Exception('Nieprawidłowa opcja wybrana dla dropdown: ' . htmlspecialchars($value));
				}
			}
